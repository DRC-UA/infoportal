...
import {KoboAnswerFlat, KoboAnswerId, KoboIndex, Meal_pdmStandardised, Period} from '@infoportal-common'
...
export interface MealPdmContext {
  fetcherAnswers: UseFetcher<(filter: Partial<Period>) => Promise<Seq<InferTypedAnswer<'meal_pdmStandardised'>>>>
  fetcherPeriod: UseFetcher<() => Promise<Period>>
  periodFilter: Partial<Period>
  setPeriodFilter: Dispatch<SetStateAction<Partial<Period>>>
  answersIndex?: Record<KoboAnswerId, KoboAnswerFlat<Meal_pdmStandardised.T, any>>
}
...
export const MealPdmProvider = ({
  ...
  const request = (filter: Partial<Period>) => api.kobo.typedAnswers.search.meal_pdmStandardised({
    filters: {
      start: filter.start,
      end: filter.end,
    }
  ...
  const fetcherPeriod = useFetcher(() => api.kobo.answer.getPeriod(KoboIndex.byName('meal_pdmStandardised').id))
