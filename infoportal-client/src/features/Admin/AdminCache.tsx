...
export const AdminCache = () => {
  ...
  const asyncClear = useAsync((k: string, sk?: string) => api.cache.clear(k, sk).then(() => fetcherCache.fetch({force: true, clean: false})), {
            ...
            <Panel key={key}>
              ...
              <PanelBody>
                <Txt block color="hint" sx={{mb: 1, mt: -1}}>
                  Refreshed {dateFromNow(info.lastUpdate)},
                  expire {dateFromNow(addMilliseconds(info.lastUpdate, info.expiration ?? 0))}
                </Txt>
                <Box sx={{}}>
                  {Object.entries(info.value).map(([subKey, value]) => (
                    <Box key={key + subKey}>
                      <Box sx={{
                        ...
                        <Txt block bold sx={{width: '100%',}}>{subKey}</Txt>
                        <IpIconBtn
                          color="primary"
                          size="small"
                          children="delete"
                          loading={asyncClear.loading[key + subKey]} onClick={() => asyncClear.call(key, subKey)}
                        />
                      </Box>
                      <Box sx={{
                        overflow: 'hidden',
