...
export const MpcaBudgetTracker = ({
  ...
  const canEditFetcher = useFetcher(() => session.admin
    ? Promise.resolve(true)
    : api.group.getMine().then(_ => !!_.find(_ => _.name === appConfig.mpcaBudgetHolderGroupName)) as Promise<boolean>
  )
          ...
          content={() => (
            <Box sx={{width: 280, pt: 1}}>
              {errorFormAlreadyExists && (
                <Alert color="error" sx={{mb: 1}}>{m.mpca.errorFormAlreadyExists}</Alert>
              )}
              <Controller
                    ...
                    <PopoverWrapper content={(close) => {
                      ...
                      return (
                        <Box sx={{p: 1,}}>
                          ...
                          <Box sx={{mt: 1,}}>
                            <IpBtn size="small" onClick={close}>{m.close}</IpBtn>
                            <IpBtn
                              size="small"
                              variant="contained"
                              ...
                              onClick={updateBudgetForm.handleSubmit((form) => {
                                asyncBudgetUpdate.call(fetcherBudget.get!.map(_ => {
                                  if (_.project === row.project && _.office === row.office) {
                                    _.budget = form.budget
                                  }
                                  return _
                                })).then(() => fetcherBudget.fetch({clean: false, force: true})).then(close)
                              })}
                            >
                              {m.confirm}
                            </IpBtn>
