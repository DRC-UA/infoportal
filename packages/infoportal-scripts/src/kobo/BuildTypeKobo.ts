  ...
  static readonly config = Obj.map(
    {
      ...
      protection_counselling: {
        formId: KoboIndex.byName('protection_counselling').id,
        skipQuestionTyping: ['ben_det_hromada', 'ben_det_raion'],
      },
      protection_referral: {
      ...
      shelter_cashForShelter: {
        formId: KoboIndex.byName('shelter_cashForShelter').id,
        skipQuestionTyping: ['ben_det_hromada', 'ben_det_raion'],
      },
        ...
        overrideOptionsByQuestion: {
          what_are_the_barriers_to_accessing_health_services: {
            safety_risks_associated_with_access_to_presence_at_health_facility: [
              'Safety risks associated with access to/presence at health facility',
            ],
          },
          what_are_your_main_concerns_regarding_your_accommodation: {
            ...
            lack_of_financial_compensation_or_rehabilitation_for_damage_or_destruction_of_housing: [
              `Lack of support for damaged housing`,
            ],
          },
      ...
      protection_hhs3: {
        ...
        overrideOptionsByQuestion: {
          ...
          what_do_you_think_feel_are_the_major_stress_factors_for_you_and_your_household_members: {
            fear_of_property_being_damaged_or_destroyedby_armed_violence: [
              `Fear of property being damaged by armed violence`,
            ],
          },
          ...
          why_did_you_decide_to_return_to_your_area_of_origin: {
            ...
            increased_restored_service_availability_in_area_of_origin: [
              `Increased/restored service availability in area of origin`,
            ],
            increased_restored_access_to_livelihood_employment_and_economic_opportunities_in_area_of_origin: [
              `Increased access to livelihood in area of origin`,
            ],
            repaired_restored_infrastructure_in_area_of_origin: [`Repaired/restored infrastructure in area of origin`],
            repaired_housing_compensation_for_destroyedor_damaged_property_in_area_of_origin: [
              `Repaired property in area of origin`,
            ],
            ...
            social_tensions_and_conflict_with_host_community_in_area_of_displacement: [
              `Social tensions and conflict with host community`,
            ],
            ...
            lack_of_access_to_livelihoods_employment_and_economic_opportunities_in_area_of_displacement: [
          ...
          what_are_the_barriers_to_accessing_health_services: {
            safety_risks_associated_with_access_to_presence_at_health_facility: [
              'Safety risks linked with access to/presence at facilities',
            ],
          },
      ...
      bn_0_mpcaRegNewShort: {
        formId: KoboIndex.byName('bn_0_mpcaRegNewShort').id,
        skipQuestionTyping: ['hromada', 'raion'],
      },
      bn_0_mpcaReg: {
        formId: KoboIndex.byName('bn_0_mpcaReg').id,
        skipQuestionTyping: ['hromada', 'raion'],
      },
      bn_0_mpcaRegNoSig: {
        formId: KoboIndex.byName('bn_0_mpcaRegNoSig').id,
        skipQuestionTyping: ['hromada', 'raion'],
      },
      bn_0_mpcaRegESign: {
        formId: KoboIndex.byName('bn_0_mpcaRegESign').id,
        skipQuestionTyping: ['hromada', 'raion'],
      },
      bn_re: {
...
class KoboInterfaceGenerator {
  ...
  readonly excludedQuestionNames = ['start', 'end']
  ...
  readonly generate = async () => {
    const form = await this.sdk.v2.survey.get({formId: this.options.formId, use$autonameAsName: true})
...
const extractQuestionName = (_: Record<string, any>) => {
    ...
    const properties = survey
      ...
      .map((x) => {
        ...
        const basicQuestionTypeMapping = (lastQuestionNameHavingOptionId?: string) => ({
          select_one: () =>
            'undefined | ' +
            (this.options.skipQuestionTyping?.includes(x.name!)
              ? 'string'
              : `Option<'${lastQuestionNameHavingOptionId}'>`),
          select_multiple: () =>
            'undefined | ' +
            (this.options.skipQuestionTyping?.includes(x.name!)
              ? 'string[]'
  ...
  readonly generateOptionsType = ({
    ...
    const indexOptionId = seq(survey).reduceObject<Record<string, string>>((_) => [
      _.select_from_list_name ?? '',
      _.name ?? '',
    ])
    ...
    choices?.forEach((choice) => {
      ...
      res[questionName][choice.name] = (() => {
        try {
          return (
            this.options.overrideOptionsByQuestion?.[questionName]?.[choice.name]?.[0] ??
