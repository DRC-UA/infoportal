...
export namespace KoboGeneralMapping {
  ...
  } & Partial<Pick<XlsIndividualBase, 'hh_char_hh_det_dis_select' | 'hh_char_hh_det_dis_level'>>

  export type XlsKoboIndividuals = Partial<
    Pick<
  ...
  export type XlsDisplacementStatus =
    | 'idp'
    | 'returnee'
    | 'non-displaced'
    | 'unspec'
  ...
  export const mapPersonDetails = (p: {
    ...
    if (p.hh_char_hh_det_disability !== undefined) {
      res.disability = seq(p.hh_char_hh_det_disability ?? [])
        .map((_) =>
          fnSwitch(
            _!,
            {
    ...
    if (disabilityLevel !== undefined && disabilityLevel !== 'zero')
      res.disability = seq(disabilitySelect ?? [])
        .map((_) =>
          fnSwitch(
            _!,
            {
  ...
  export const addIndividualBreakdownColumn = <T extends XlsKoboIndividuals>(
    row: T,
  ): T & {custom: IndividualBreakdown} => {
  ...
  export const addIndividualBreakdownColumnForRrm = (
    row: Bn_rapidResponse.T,
  ): Bn_rapidResponse.T & {custom: IndividualBreakdown} => {
  ...
  export const collectXlsKoboIndividualsFromStandardizedKoboForm = (d: XlsKoboIndividuals): XlsKoboIndividual[] => {
    return (
      (d.hh_char_hh_det ?? d.hh_member ?? []).map((_) => {
