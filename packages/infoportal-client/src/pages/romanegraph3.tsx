...
export const Pan = ({title, children}: {title: string; children: ReactNode}) => {
  return (
    <Box
      sx={{
        maxWidth: 500,
    ...
    >
      <Txt block size="big" bold sx={{textAlign: 'center', mt: 2, mb: 1}}>
        {title}
      </Txt>
...
const optionsUa: Partial<Record<keyof typeof Protection_hhs3.options, Record<string, string>>> = {
  ...
  what_do_you_think_feel_are_the_major_stress_factors_for_you_and_your_household_members: {
    ...
    fear_of_property_being_damaged_or_destroyedby_armed_violence:
      'Страх за можливість пошкодження або руйнування майна через збройний конфлікт',
  ...
  what_are_your_main_concerns_regarding_your_accommodation: {
    ...
    lack_of_financial_compensation_or_rehabilitation_for_damage_or_destruction_of_housing:
      'Відсутність пітримки відновлення житла',
  ...
  what_are_the_barriers_to_accessing_health_services: {
    ...
    distance_lack_of_transportation_means_to_access_facilities:
      'Відстань - нестача засобів пересування, щоб дістатися до медичних закладів',
    ...
    safety_risks_associated_with_access_to_presence_at_health_facility:
      'Ризики безпеки повязані з доступом/перебуванням в медичному закладі',
...
export default () => {
  ...
  const ageGroup = useCallback(
    (ageGroup: Person.AgeGroup, hideOther?: boolean) => {
  ...
  return (
    <Page loading={fetcher.loading} sx={{background: t.palette.background.paper}}>
      ...
      <Pan title={title('Intentions per displacement status')}>
        <Txt bold block color="hint" size="small">
          {m.idps}
        </Txt>
        <ChartBarSingleBy
        ...
        />
        <Txt bold block color="hint" size="small" sx={{mt: 3}}>
          {m.nonDisplaced}
        </Txt>
        <ChartBarSingleBy
        ...
        />
        <Txt bold block color="hint" size="small" sx={{mt: 3}}>
          {m.refugeesAndReturnees}
        </Txt>
        <ChartBarSingleBy
          data={data.filter(
            (_) =>
              _.do_you_identify_as_any_of_the_following === 'refugee' ||
              _.do_you_identify_as_any_of_the_following === 'returnee',
          )}
          filter={(_) =>
            _.what_are_your_households_intentions_in_terms_of_place_of_residence !== 'unable_unwilling_to_answer'
          }
      ...
      <Pan title={title('Major stress factors')}>
        <ChartBarMultipleBy
          ...
          label={{
            ...
            ...(lang === 'en'
              ? {}
              : optionsUa.what_do_you_think_feel_are_the_major_stress_factors_for_you_and_your_household_members),
          }}
        />
      </Pan>
