...
export const MpcaDashboardDeduplication = ({data}: {data: Seq<MpcaEntity>}) => {
  const {m} = useI18n()
  ...
  return (
    <Panel>
      <PanelHead
        action={
          <ScRadioGroup inline dense value={curveType} onChange={setCurveType}>
            <ScRadioGroupItem hideRadio value="ratio">
              %
            </ScRadioGroupItem>
            <ScRadioGroupItem hideRadio value="value">
              {m.value}
            </ScRadioGroupItem>
          </ScRadioGroup>
        }
      ...
      <PanelBody>
        <Lazy
          deps={[data, curveType, displayedCurves]}
          fn={() => {
        ...
        <Box sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
          {Obj.keys(defaultDisplayedCurves).map(
            (key) =>
              !(key === 'Total' && curveType === 'ratio') && (
                <Txt size="small" key={key}>
                  <Checkbox
                    size="small"
                    sx={{ml: 0.5, '& svg': {fill: colors(theme)[key] + ' !important'}}}
