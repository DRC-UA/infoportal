  ...
  const filterShape = useMemo(() => {
    return DataFilter.makeShape<PdmData<Meal_cashPdm.T>>({
      ...
      pdmtype: {
        ...
        getValue: _ =>
          _.answers.pdmtype && _.answers.pdmtype.length > 0
            ? _.answers.pdmtype[0]
            : undefined,
      },
    })
          ...
          <Div responsive>
            ...
            <Div column sx={{maxHeight: '50%'}}>
              <SlidePanel title={m.mealMonitoringPdm.betterInform}>
                <ChartBarMultipleBy data={data} by={_ => _.answers.better_inform_distribution} label={Meal_cashPdm.options.better_inform_distribution}/>
              </SlidePanel>
              ...
              <SlidePanel title={m.mealMonitoringPdm.timeToTake}>
                <ChartBarSingleBy data={data} by={_ => _.answers.time_registered_assistance} label={Meal_cashPdm.options.time_registered_assistance}/>
              </SlidePanel>
              <SlidePanel title={m.mealMonitoringPdm.assistanceReceive}>
                <ChartBarSingleBy data={data} by={_ => _.answers.assistance_delivered} label={Meal_cashPdm.options.assistance_delivered}/>
              </SlidePanel>
              <SlidePanel title={m.mealMonitoringPdm.priorityNeeds}>
                <ChartBarMultipleBy data={data} by={_ => _?.answers?.needs_community_currently} label={Meal_cashPdm.options?.needs_community_currently}/>
              </SlidePanel>
            </Div>
          </Div>
        </>
