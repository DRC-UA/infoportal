...
const AnswersRow: FC<{answer: string; answerStats: Record<Person.Gender, Record<string, number>>}> = ({
  ...
  return (
    <TableRow>
      ...
      <TableCell>{answerStats?.Female?.['5 - 17'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Female?.['18 - 49'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Female?.['50+'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Female?.Disability ?? 0}</TableCell>
      <TableCell>{answerStats?.Male?.['5 - 17'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Male?.['18 - 49'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Male?.['50+'] ?? 0}</TableCell>
      <TableCell>{answerStats?.Male?.Disability ?? 0}</TableCell>
      <TableCell>[ diversity group a ]</TableCell>
      <TableCell>[ diversity group b ]</TableCell>
      <TableCell>
        {seq(Object.values(answerStats?.Female ?? {})).sum((subCount: number | undefined) => safeNumber(subCount, 0))}
      </TableCell>
      <TableCell>
        {seq(Object.values(answerStats?.Male ?? {})).sum((subCount: number | undefined) => safeNumber(subCount, 0))}
      </TableCell>
      <TableCell>
        {safeNumber(answerStats?.Female?.['5 - 17'], 0) + safeNumber(answerStats?.Male?.['5 - 17'], 0)}
      </TableCell>
      <TableCell>
        {safeNumber(answerStats?.Female?.['18 - 49'], 0) + safeNumber(answerStats?.Male?.['18 - 49'], 0)}
      </TableCell>
      <TableCell>{safeNumber(answerStats?.Female?.['50+'], 0) + safeNumber(answerStats?.Male?.['50+'], 0)}</TableCell>
      <TableCell>
        {safeNumber(answerStats?.Female?.Disability, 0) + safeNumber(answerStats?.Male?.Disability, 0)}
      </TableCell>
    </TableRow>
  )
}
  ...
  <TableHead>
    ...
    <TableRow>
      ...
      {echoAgeGroups.map((echoAgeGroup) => (
        <TableCell key={`Female ${echoAgeGroup}`}>{echoAgeGroup}</TableCell>
      ))}
      <TableCell>Living with disability</TableCell>
      {echoAgeGroups.map((echoAgeGroup) => (
        <TableCell key={`Male ${echoAgeGroup}`}>{echoAgeGroup}</TableCell>
      ))}
      <TableCell>Living with disability</TableCell>
      <TableCell>Diversity group A</TableCell>
      <TableCell>Diversity group B</TableCell>
      ...
      <TableCell>Total 5-17 years</TableCell>
      <TableCell>Total 18-49 years</TableCell>
      <TableCell>Total 50 years and more</TableCell>
      <TableCell>Total people living with disability</TableCell>
    </TableRow>
  </TableHead>
)

const EchoTableBody: FC<EchoPmKoiTableData> = (data) => {
  return (
    <TableBody>
