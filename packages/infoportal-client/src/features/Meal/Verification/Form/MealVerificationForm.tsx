...
const RenderRow = ({icon, label, children}: {icon?: string; label: ReactNode; children: ReactNode}) => {
  return (
    <Box sx={{display: 'flex'}}>
      <Box sx={{width: 30, textAlign: 'center', mr: 1}}>{icon && <Icon color="disabled">{icon}</Icon>}</Box>
      <Box sx={{mb: 2}}>
        <Txt bold block sx={{mr: 2, mb: 0.5}}>
          {label}
        </Txt>
...
export const MealVerificationForm = () => {
  ...
  const BackBtn = ({}: {}) => (
    <IpBtn onClick={prevStep} sx={{mt: 1, mr: 1}}>
      {m.back}
    </IpBtn>
  )
  ...
  const NextBtn = ({
    ...
    <IpBtn disabled={disabled} variant="contained" onClick={nextStep} {...props} sx={{mt: 1, mr: 1}}>
      {label ?? m.continue}
    </IpBtn>
  )
        ...
        <PanelBody>
          <Stepper nonLinear activeStep={activeStep} orientation="vertical">
                          ...
                          description={
                            <Box sx={{display: 'flex', alignItems: 'center'}}>
                              <Icon fontSize="small" sx={{mr: 0.5}}>
                                {appConfig.icons.koboForm}
                              </Icon>
                              {
                                KoboIndex.searchById(
                                  mealVerificationActivitiesIndex[activity.id].registration.koboFormId,
                                )?.translation
                              }
                            </Box>
                          }
            ...
            <Step completed={!!form.watch('answerIds')}>
              ...
              <StepContent>
                <Txt color="hint" sx={{mb: 1}}>
                  {m._mealVerif.applyFilters}
                </Txt>
                {activity && (
                ...
                <Box sx={{mb: 1}}>
                  <NextBtn
                    disabled={form.watch('answerIds')?.length === undefined}
                    label={m._mealVerif.selectedNRows(form.watch('answerIds')?.length ?? '-')}
                  />
                  <BackBtn />
                </Box>
              </StepContent>
            ...
            <Step completed={!!form.watch('name')}>
              ...
              <StepContent>
                <Controller
                  ...
                  rules={{required: {value: true, message: m.required}}}
                  control={form.control}
                  render={({field}) => <IpInput {...field} label={m._mealVerif.giveANameToId} />}
                />
                <Controller
                  name="desc"
                  control={form.control}
                ...
                {activity && (
                  <>
                    ...
                    <RenderRow label={m._mealVerif.selectedKoboForm} icon="fact_check">
                      ...
                      <Txt block>
                        ...
                        <Box component="span" sx={{fontFamily: 'monospace'}}>
                          {KoboIndex.searchById(activity.registration.koboFormId)?.translation}
                        </Box>
                      </Txt>
                    </RenderRow>
                    ...
                    <RenderRow label={m.data} icon="table_view">
                      <Box
                        dangerouslySetInnerHTML={{__html: m._mealVerif.selectedData(form.watch().answerIds?.length)}}
                      />
                      <Box sx={{mt: 0.5}}>
                        <Icon color="disabled" sx={{verticalAlign: 'top', mr: 1}}>
                          subdirectory_arrow_right
                        </Icon>
                        {m._mealVerif.sampleSizeN(activity.sampleSizeRatio * 100)}
                        <Icon color="disabled" sx={{verticalAlign: 'top', mx: 1}}>
                          east
                        </Icon>
                        <Txt
                          sx={{
                          ...
                          dangerouslySetInnerHTML={{
                            __html: m._mealVerif.dataToBeVerified(
                              Math.floor(activity.sampleSizeRatio * form.watch().answerIds?.length),
                            ),
                          }}
                        />
                      </Box>
