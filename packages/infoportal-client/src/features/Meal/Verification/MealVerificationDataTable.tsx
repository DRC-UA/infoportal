...
import {
  MealVerificationActivity,
  mealVerificationConf,
  VerifiedColumnsMapping,
...
export type MealVerificationBundle<TReg extends KoboFormNameMapped = any, TVerif extends KoboFormNameMapped = any> = {
...
>({
  ...
  const {mergedData, duplicateErrors, unselectedAnswers} = useMemo(() => {
      ...
      .flatMap((rowReg: InferTypedAnswer<TReg>) => {
        ...
        return (rowVerifs ? rowVerifs : [undefined]).map((rowVerif) => {
          ...
          const res: ComputedRow = {
            ...mergedData,
            score: mergedData.verifiedData
              ? seq(Obj.values(mergedData.verifiedData)).sum((_) => (_.equals ? 1 : 0))
              : 0,
          }
          return res
        })
    ...
    return {
      mergedData,
      duplicateErrors,
      unselectedAnswers: bundle.toVerify
        .filter((_) => _.status !== MealVerificationAnswersStatus.Selected)
        ...
        <Div sx={{mb: 1, alignItems: 'stretch'}}>
          ...
          <SlidePanel sx={{flex: 1}}>
            <ChartPieWidget
              ...
              showValue
              showBase
              value={stats?.verifiedRows.length ?? 0}
              base={stats.selectedRows?.length ?? 1}
              title={m._mealVerif.verified}
            />
          </SlidePanel>
          <SlidePanel sx={{flex: 1}}>
            <ChartPieWidget
              ...
              base={stats?.indicatorsCount ?? 1}
              title={m._mealVerif.valid}
            />
          </SlidePanel>
          ...
          columns={[
            {
              ...
              renderQuick: (_) => {
                ...
                return (
                  <>
                    <TableIconBtn
                      tooltip={m._mealVerif.viewRegistrationData}
                      children="text_snippet"
                      onClick={() => setOpenModalAnswer(_.rowReg)}
                    />
                    <TableIconBtn
                      tooltip={m._mealVerif.viewDataCheck}
                      disabled={!_.rowVerif}
                      children="fact_check"
                        ...
                        _.status,
                        {
                          NotSelected: (
                            <TableIconBtn
                              ...
                              onClick={() =>
                                asyncUpdateAnswer
                                  .call(verif.id, MealVerificationAnswersStatus.Selected)
                              ...
                              <TableIconBtn
                                ...
                                onClick={() => {
                                  Promise.all([
                                    ...
                                    asyncUpdateAnswer.call(
                                      unselectedAnswers.pop()?.id!,
                                      MealVerificationAnswersStatus.Selected,
                                    ),
                                  ]).then(refreshToVerify)
                                }}
                              />
            ...
            ...Obj.entries(harmonizedVerifiedColumns).map(([id, c]) => {
              return {
                ...
                style: (_: ComputedRow) => {
                  if (!_.verifiedData[id].valueVerif)
                    return {
                  ...
                  if (!_.rowVerif || _.verifiedData[id].equals) return {}
                    ...
                    return {
                      ...
                      background: alpha(t.palette.error.main, 0.08),
                    }
                },
