  ...
  const filterShape = useMemo(() => {
    return DataFilter.makeShape<KoboSubmissionFlat<Meal_visitMonitoring.T>>({
      oblast: {
        icon: 'location_on',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('mdro').map((_) => ({value: _.name, label: _.label[langIndex]})),
        label: m.office,
        getValue: (_) => _.mdro,
      },
      focalPoint: {
        icon: 'person',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('mdp').map((_) => ({value: _.name, label: _.label[langIndex]})),
        label: m.focalPoint,
        getValue: (_) => _.mdp,
      },
      donor: {
      ...
      activity: {
        ...
        icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('mdt').map((_) => ({value: _.name, label: _.label[langIndex]})),
        label: m.project,
        getValue: (_) => _.mdt,
      },
      nfi: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pan').map((_) => ({value: _.name, label: _.label[langIndex]})),
      ...
      ecrec: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pae').map((_) => ({value: _.name, label: _.label[langIndex]})),
      ...
      shelter: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pas').map((_) => ({value: _.name, label: _.label[langIndex]})),
      ...
      lau: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pal').map((_) => ({value: _.name, label: _.label[langIndex]})),
      ...
      protection: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pap').map((_) => ({value: _.name, label: _.label[langIndex]})),
      ...
      eore: {
        // icon: 'edit_calendar',
        getOptions: () =>
          schema.helper.getOptionsByQuestionName('pao').map((_) => ({value: _.name, label: _.label[langIndex]})),
          ...
          <Div responsive>
            <Div column>
              ...
              <SlidePanel title={m.donor}>
                <ChartBarMultipleBy label={Meal_visitMonitoring.options.mdd_001} data={data} by={(_) => _.mdd_001} />
              </SlidePanel>
              <SlidePanel>
                <ChartPieWidgetBy
                  title={m.mealMonitoringVisit.securityConcerns}
                  filter={(_) => _.ssy === 'yes'}
                  data={data}
                  sx={{mb: 1}}
                />
              ...
              <SlidePanel>
                <ChartPieWidgetBy
                  title={m.mealMonitoringVisit.concerns}
                  filter={(_) => _.sef === 'yes'}
                  data={data}
                  sx={{mb: 1}}
                />
              ...
              <SlidePanel>
                <ChartPieWidgetBy
                  title={m.mealMonitoringVisit.criticalConcern}
                  filter={(_) => _.visf === 'yes'}
                  data={data}
                  sx={{mb: 1}}
                />
