...
} & Omit<TableIconProps, 'children'>) =>
  fnSwitch(status, {
    [KoboMealCfmStatus.Close]: (
      <TableIcon {...props} tooltip="Close" color="success">
        check_circle
      </TableIcon>
    ),
    [KoboMealCfmStatus.Open]: (
      <TableIcon {...props} tooltip="Open" color="warning">
        new_releases
      </TableIcon>
    ),
    [KoboMealCfmStatus.Processing]: (
      <TableIcon {...props} tooltip="Processing" color="info">
        schedule
      </TableIcon>
    ),
    [KoboMealCfmStatus.Archived]: (
      <TableIcon {...props} tooltip="Processing" color="disabled">
        archive
      </TableIcon>
    ),
  })

export const CfmStatusIconLabel = ({
  status,
  ...props
}: {
  status: KoboMealCfmStatus
...
export type CfmData = {
  ...
  KoboSubmissionFlat<Meal_cfmInternal.T>,
  // 'ben_det_oblast' |
  'ben_det_raion' | 'ben_det_hromada'
> &
  Pick<
    KoboSubmissionFlat<Meal_cfmInternal.T>,
    'id' | 'start' | 'date' | 'end' | 'submissionTime' | 'name' | 'gender' | 'phone' | 'email'
  >
...
export interface CfmContext {
  ...
  asyncRemove: UseAsyncMultiple<(_: {formId: Kobo.FormId; answerId: Kobo.SubmissionId}) => Promise<void>, Kobo.FormId>
...
export const CfmProvider = ({
  ...
  const authorizations: CfmContext['authorizations'] = useMemo(() => {
    ...
    const accessibleOffices = cfmAccesses
      .map((_) => _.params)
      .flatMap((_) => _?.office)
      .compact()
      .get()
    const accessiblePrograms = cfmAccesses
      .map((_) => _.params)
      .flatMap((_) => _?.program)
      .compact()
      .get()
  ...
  const mappedData = useMemo(() => {
    return (
      map(fetcherInternal.get, fetcherExternal.get, (internal, external) => {
        const res: CfmData[] = []
  ...
  }, [fetcherInternal.get, fetcherExternal.get])

  const visibleData = useMemo(() => {
    return mappedData?.filter((_) => {
      if (_.tags?.deletedBy) return false
      if (session.email === _.tags?.focalPointEmail) return true
      if (!authorizations.sum.read) return false
      if (authorizations.accessiblePrograms && !authorizations.accessiblePrograms.includes(_.tags?.program!))
        return false
      if (authorizations.accessibleOffices && !authorizations.accessibleOffices.includes(_.tags?.office!)) return false
  ...
  const asyncRemove = useAsync(
    async ({formId, answerId}: {formId: Kobo.FormId; answerId: Kobo.SubmissionId}) => {
