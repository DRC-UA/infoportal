...
export const CfmPriorityLogo = ({
  ...
  return fnSwitch(
    priority!,
    {
      ...
      Medium: (
        <TableIcon tooltip={m._cfm.priority + ': ' + priority} sx={sx} fontSize={fontSize} color="warning">
          looks_two
        </TableIcon>
      ),
      High: (
        <TableIcon tooltip={m._cfm.priority + ': ' + priority} sx={sx} fontSize={fontSize} color="error">
          looks_one
        </TableIcon>
      ),
    },
...
export const CfmTable = ({}: any) => {
  ...
  const defaultFilters = useMemo(() => {
    return {
      status: [KoboMealCfmStatus.Processing, KoboMealCfmStatus.Open, KoboMealCfmStatus.Close],
    }
  }, [])
  return (
    <Page width="full">
        ...
        <Datatable
          ...
          columns={[
                  ...
                  label: (
                    <IpSelectSingle
                      ...
                      options={Obj.keys(KoboMealCfmStatus)
                        ...
                        .map((_) => ({
                          value: _,
                          children: <CfmStatusIcon status={KoboMealCfmStatus[_]} />,
                        }))}
                    ...
                    <IpSelectSingle
                      ...
                      onChange={(newValue) => {
                        ctxKoboUpdate.asyncUpdateById.answer.call({
                          formId: row.formId,
                          answerIds: [row.id],
                          question: 'benef_origin',
                          answer: newValue,
                        })
                      }}
                      options={Obj.entries(Meal_cfmInternal.options.benef_origin).map(([k, v]) => ({
                        value: k,
                        children: v,
                      }))}
                    />
                        ...
                        <Autocomplete
                          ...
                          renderInput={({InputProps, ...props}) => (
                            <IpInput
                              ...
                              endAdornment={
                                value &&
                                !Regexp.get.drcEmail.test(value) && (
                                  <TableIcon tooltip={m.invalidEmail} color="error">
                                    error
                                  </TableIcon>
                                )
                              }
            ...
            },
            {
              ...
              id: 'gender',
              options: () =>
                Obj.keys(Meal_cfmInternal.options.gender).map((value) => ({
                  value,
                  label: ctx.schemaExternal.translate.choice('gender', value),
                })),
              render: (_) => {
                return {
                  value: _.gender,
                  ...
                  {(ctx.authorizations.sum.write || session.email === row.tags?.focalPointEmail) && (
                    <>
                      ...
                      <Modal
                        ...
                        onConfirm={(e, close) =>
                          ctx.asyncRemove.call({formId: row.formId, answerId: row.id}).then(close)
                        }
