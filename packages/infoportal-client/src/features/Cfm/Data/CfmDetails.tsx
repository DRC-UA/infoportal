...
export const CfmEntryRoute = () => {
  ...
  const entry = useMemo(
    () => ctx.mappedData?.find((_) => _.id === answerId && formId === formId),
    [formId, ctx.mappedData],
  )
  ...
  return <Page loading={ctx.fetching}>{entry && <CfmDetails entry={entry} />}</Page>
}

export const CfmDetails = ({entry}: {entry: CfmData}) => {
  ...
  return (
    <Page>
      ...
      >
        <Box sx={{display: 'flex', alignItems: 'center'}}>
          <NavLink to={cfmIndex.siteMap.data}>
            <IpIconBtn color="primary" sx={{mr: 1}}>
              arrow_back
            </IpIconBtn>
          </NavLink>
        ...
        <Grid item xs={6}>
          <Panel>
            ...
            <PanelBody>
              ...
              <Divider />
              <ListRow icon="location_on" label={m.oblast}>
                {entry.oblast}
              </ListRow>
              <ListRow icon="" label={m.raion}>
                {ctx.schemaExternal.translate.choice('ben_det_raion', entry.ben_det_raion)}
              </ListRow>
              <ListRow icon="" label={m.hromada}>
                {ctx.schemaExternal.translate.choice('ben_det_hromada', entry.ben_det_hromada)}
              </ListRow>
              {entry.form === CfmDataSource.Internal ? (
                <>
                  <Divider />
                  <ListRow icon="bookmark" label={m._cfm.existingDrcBeneficiary}>
                    {entry.benef_origin}
                  </ListRow>
                  <ListRow icon="" label={m.projectCode}>
                    {entry.project}
                  </ListRow>
                </>
              ) : (
              ...
              <ListRow icon="support_agent" label={m.focalPoint}>
                <TableInput
                  ...
                  helper={(() => {
                    ...
                    if (email && !Regexp.get.drcEmail.test(email))
                      return {
                        text: m.invalidEmail,
                        status: 'error',
                      }
      ...
      <Panel>
        <PanelHead>
          {m._cfm.feedback}{' '}
          {entry.external_feedback_type ? `(${m._cfm._feedbackType[entry.external_feedback_type!]})` : ``}
        </PanelHead>
        <PanelBody>
          <IpSelectSingle
            sx={{mb: 2}}
          ...
          {isEditing ? (
            <Box sx={{textAlign: 'right'}}>
              ...
              <IpBtn color="error" icon="cancel" size="small" variant="outlined" onClick={() => setIsEditing(false)}>
                {m.cancel}
              </IpBtn>
            </Box>
          ) : (
            <IpBtn icon="edit" size="small" variant="outlined" onClick={() => setIsEditing(true)}>
              {m.edit}
            </IpBtn>
          )}
        </PanelBody>
      ...
      {ctx.authorizations.sum.admin && (
        <Box sx={{display: 'flex', justifyContent: 'center'}}>
          <Modal
            ...
            onConfirm={() =>
              ctx.asyncRemove
                .call({formId: entry.formId, answerId: entry.id})
                .then(() => navigate(cfmIndex.siteMap.data))
            }
            title={m.shouldDelete}
          >
            <IpBtn icon="delete" size="large" sx={{margin: 'auto'}} variant="contained">
              {m.remove}
            </IpBtn>
          </Modal>
        </Box>
