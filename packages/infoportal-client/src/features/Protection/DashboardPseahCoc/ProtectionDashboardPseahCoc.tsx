...
import {OblastIndex, Period, PeriodHelper, Protection_coc, Pseah_coc} from 'infoportal-common'
...
export const ProtectionDashboardPseahCoc = () => {
  ...
  const fetcherCoc = useFetcher(api.kobo.typedAnswers.search.pseah_coc)
  useEffect(() => {
    fetcherCoc.fetch()
  }, [])
  ...
  const filterShape = useMemo(
    () =>
      DataFilter.makeShape<InferTypedAnswer<'pseah_coc'>>({
        office: {
          icon: conf.icons.oblast,
          getValue: (_) => _.office_staff_trained,
          getOptions: () =>
            Obj.entries(Pseah_coc.options.office_staff_trained).map(([k, v]) => DataFilter.buildOption(k, v)),
          label: m.office,
        },
        modality: {
          ...
          getOptions: () =>
            Obj.entries(Pseah_coc.options.modality_training).map(([k, v]) => DataFilter.buildOption(k, v)),
          label: m.modality,
        },
        typeTraining: {
          icon: appConfig.icons.program,
          getValue: (_) => _.type_training,
          getOptions: () =>
            Obj.entries(Pseah_coc.options.type_training).map(([k, v]) => DataFilter.buildOption(k, v)),
          label: m.typeTraining,
        },
      }),
    [m],
  )
  ...
  const {listDone, listNotIdentified} = useMemo(() => {
    ...
    const arr = [
      ...
      ...dataFiltered
        .flatMap((_) =>
          _.training_eparticipants?.map((participants) => {
            if (_.email) return _.email
            if (_.name) {
              ...
              if (!match) {
                const names = _.name.split(' ').map((_) => _.toLowerCase())
              ...
              if (!match) listNotIdentified.push(_.name)
              return match
            }
          }),
        )
