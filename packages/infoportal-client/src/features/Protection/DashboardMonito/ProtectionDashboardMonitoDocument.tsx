  ...
  return base
    ...
    .filter(
      (_) =>
        ...
        (_.are_you_and_your_hh_members_registered_as_idps &&
          _.are_you_and_your_hh_members_registered_as_idps !== 'unable_unwilling_to_answer'),
    )
}
...
export const ProtectionDashboardMonitoDocument = () => {
  ...
  const filterPerson = (_: Seq<KoboProtection_hhs3.Person>) =>
    _.filter((_) => gender.length === 0 || gender.includes(_.gender!)).filter((_) => {
      ...
      <Div responsive alignItems="flex-start">
                ...
                fn={(d) =>
                  ChartHelper.percentage({
                    data: getIdpsAnsweringRegistrationQuestion(d).filter(
                      (_) => _.age && _.age >= 25 && _.age <= 60 && _.gender && _.gender === Person.Gender.Male,
                    ),
                    value: (_) =>
                      _.isIdpRegistered !== 'yes' && _.are_you_and_your_hh_members_registered_as_idps !== 'yes_all',
                  })
        ...
        <Div column sx={{flex: 1}}>
          <SlidePanel title={m.lackOfPersonalDoc}>
            ...
            <Lazy
              deps={[filteredPersons, filteredPersonsLastMonth]}
              fn={(x) =>
                ChartHelper.percentage({
                  data: x
                    .map((_) => _.lackDoc)
            ...
            >
              {(_, last) => (
                <ChartPieWidget
                  dense
                  sx={{mb: 2}}
                  evolution={(_?.percent ?? 1) - (last?.percent ?? 1)}
                  value={_.value}
                  base={_.base}
                />
              )}
            ...
            <ChartBarMultipleBy
              ...
              label={{
                ...
                document_issues_by_local_self_government_proving_that_the_house_was_damaged_destroyed:
                  'Document issued by local self-government proving a damaged house',
                ...
                document_issues_by_police_state_emergency_service_proving_that_the_house_was_damaged_destroyedfor_ukrainian_state_control_areas:
                  'Document issued by authority proving a damaged house',
              }}
            />
          </SlidePanel>
        </Div>
