...
export const SnapshotProtMonitoEchoSafety = () => {
  ...
  const groupedIndividualsType = useMemo(() => {
    const res = {
      type: seq() as Seq<
        Protection_hhs3.T['what_type_of_incidents_took_place_has_any_adult_male_member_experienced_violence']
      >,
      when: seq() as Seq<
        Protection_hhs3.T['when_did_the_incidents_occur_has_any_adult_male_member_experienced_violence']
      >,
      who: seq() as Seq<
        Protection_hhs3.T['who_were_the_perpetrators_of_the_incident_has_any_adult_male_member_experienced_violence']
      >,
    }
          ...
          <Div column>
            <SlideTxt sx={{marginBottom: t.spacing() + ' !important'}}>
                  ...
                  return {
                    ...
                    poorSafetyChernihiv: ChartHelper.percentage({
                      data: ctx.dataFiltered
                        .filter((_) => _.where_are_you_current_living_oblast === OblastIndex.byName('Chernihivska').iso)
                        .map((_) => _.please_rate_your_sense_of_safety_in_this_location),
                      value: (_) => _ === '_2_unsafe' || _ === '_1_very_unsafe',
                      base: (_) => _ !== 'unable_unwilling_to_answer',
                    }),
                    poorSafetySumy: ChartHelper.percentage({
                      data: ctx.dataFiltered
                        .filter((_) => _.where_are_you_current_living_oblast === OblastIndex.byName('Sumska').iso)
                        .map((_) => _.please_rate_your_sense_of_safety_in_this_location),
                      value: (_) => _ === '_2_unsafe' || _ === '_1_very_unsafe',
                      base: (_) => _ !== 'unable_unwilling_to_answer',
                    }),
                    senseOfSafetyRural: ChartHelper.percentage({
                      data: ctx.dataFiltered
                        .filter((_) => _.type_of_site === 'rural_area')
                        .map((_) => _.please_rate_your_sense_of_safety_in_this_location),
                      value: (_) => _ === '_2_unsafe' || _ === '_1_very_unsafe',
                      base: (_) => _ !== 'unable_unwilling_to_answer',
                    }),
                    senseOfSafetyUrban: ChartHelper.percentage({
                      data: ctx.dataFiltered
                        .filter((_) => _.type_of_site === 'urban_area')
                        .map((_) => _.please_rate_your_sense_of_safety_in_this_location),
                      value: (_) => _ === '_2_unsafe' || _ === '_1_very_unsafe',
                      base: (_) => _ !== 'unable_unwilling_to_answer',
                    }),
                    incidents: ChartHelper.percentage({
                      data: ctx.dataFiltered,
                      value: (_) =>
                        _.has_any_adult_male_member_experienced_violence === 'yes' ||
              ...
              >
                {(_) => (
                  ...
                  // }}
                  >
                    In eastern oblasts like Kharkiv and Donetsk, proximity to the frontline and frequent shelling
                    attacks create a constant sense of insecurity, while many remain reluctant to leave their homes due
                    to fears of property destruction and of mobilization. Access to safe shelters remains limited, and
                    the absence of air raid warning systems in remote areas poses significant risks to affected
                    communities. The presence of military personnel heightens fears of strikes and risks of violence,
                    further undermining residents' sense of safety.
                  </p>
                )}
              </Lazy>
            </SlideTxt>
            ...
            <SlidePanel>
              ...
              <ChartBarMultipleBy
                data={ctx.dataFiltered}
                by={(_) =>
                  _.do_you_or_your_household_members_experience_any_barriers_to_movements_in_and_around_the_area
                }
                limit={5}
                label={{
                  ...Protection_hhs3.options
