          ...
          <Div column>
            <SlideTxt>
              <Lazy
                ...
                fn={() => {
                  return {
                    fearOfShelling: toPercent(
                      ChartHelper.percentage({
                        data: ctx.dataFiltered
                          .map((_) => _.what_are_the_main_factors_that_make_this_location_feel_unsafe)
                          .compact(),
              ...
              >
                {(_) => (
                  ...
                  // }}
                  >
                    In addition to the threats posed by shelling attacks and the presence of explosive ordnance, various
                    other factors severely restrict people's freedom of movement. These include challenges related to
                    the accessibility and affordability of transportation, as well as apprehensions regarding
                    conscription. Reports persist regarding heightened concerns over active mobilization for
                    conscription, prompting men to limit their movements as a precautionary measure. Reports persist of
                    substantial stress and declining mental health and well-being, with older individuals and people
                    with disabilities being particularly susceptible to psychological distress.
                  </p>
                )}
              </Lazy>
            </SlideTxt>
            ...
            <SlidePanel>
              <Lazy
                deps={[ctx.dataFiltered.flatMap((p) => p.persons)]}
                fn={(x) =>
                  ChartHelper.percentage({
                    data: x
                      .map((_) => _.lackDoc)
              ...
              >
                {(_, last) => (
                  <ChartPieWidget title={m.lackOfPersonalDoc} dense sx={{mb: 1}} value={_.value} base={_.base} />
                )}
              </Lazy>
              <ChartBarMultipleBy
          ...
          <Div column>
            ...
            <SlidePanel>
              <ChartPieWidgetBy
                ...
                filter={(_) =>
                  !_.do_you_or_your_household_members_experience_any_barriers_to_movements_in_and_around_the_area!.includes(
                    'no',
                  )
                }
                filterBase={(_) =>
