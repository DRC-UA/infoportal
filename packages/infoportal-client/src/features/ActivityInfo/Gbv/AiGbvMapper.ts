...
export namespace AiGbvMapper {
  export type Type = Omit<AiGbvType.Type, 'Reporting Organization' | 'Response Theme'> &
    AiGbvType.TypeSub & {
  ...
  export const mapGbvActivity =
    (period: Partial<Period>) => async (res: ApiPaginate<InferTypedAnswer<'protection_gbv'>>['data']) => {
      ...
      const data: Type[] = await Promise.all(
        res
          ...
          .flatMap((d) => {
            ...
            return seq(d.custom.persons!)
              .compactBy('age')
              .sortByNumber((_) => _.age, '9-0')
              .flatMap((ind, index) => {
                return (d.activity ?? []).map(async (activity) => {
                  const res: Type = {
                    answer: d,
                    ...AiMapper.getLocationByKobo(d),
                    Settlement: await AILocationHelper.findSettlementByIso(d.ben_det_hromada_001).then(
                      (_) => _?._5w ?? aiInvalidValueFlag + d.ben_det_hromada_001,
                    ),
                    ...
                    ...(activity === 'dignity_kits' &&
                      (() => {
                        ...
                        return {
                          'Type of distribution': fnSwitch(
                            d.distribute!,
                            Protection_gbv.options.distribute,
                            () => aiInvalidValueFlag as any,
                          ),
                          'Who distributed the kits?': fnSwitch(
                            d.distributor!,
                            Protection_gbv.options.distributor,
                            () => aiInvalidValueFlag as any,
                          ),
