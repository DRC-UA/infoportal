...
export const AiBundleTable = ({
  ...
  const {activityCols, subActivityCols} = useMemo(() => {
    return {
      activityCols: seq(fetcher.get ?? [])
        ...
        .map((key) => ({
          key,
          type: fetcher.get?.some((_) => typeof _.activity[key] === 'number') ? 'number' : 'select_one',
        })),
      subActivityCols: seq(fetcher.get ?? [])
        ...
        .map((key) => ({
          key,
          type: fetcher.get?.some((_) => typeof _.subActivity[key] === 'number') ? 'number' : 'select_one',
        })),
    }
  }, [fetcher.get])
                  ...
                  {session.admin && (
                    <>
                      ...
                      <Badge
                        ...
                        overlap="circular"
                        badgeContent="!"
                        invisible={!JSON.stringify(_.requestBody).includes('undefined')}
                      >
          ...
          },
          {
            ...
            options: () => {
              return DatatableUtils.buildOptions(
                seq(fetcher.get)
                  .flatMap((_) => _.data)
                  .map((_) => KoboIndex.searchById(_.formId)?.translation ?? _.formId)
                  .distinct((_) => _)
                  .compact(),
              )
            },
            render: (_) => {
              const formIds = seq(_.data)
                .map((_) => KoboIndex.searchById(_.formId)?.translation ?? _.formId)
                .distinct((_) => _)
                .compact()
              return {
