...
import {KoboAttachedImg} from '@/shared/TableMedia/KoboAttachedMedia'
...
export const ShelterTable = () => {
  ...
  const {selectedNta, selectedTa} = useMemo(() => {
    ...
    return {
      selectedNta: seq(selected)
        .map((_) => _.nta?.id)
        .compact(),
      selectedTa: seq(selected)
        .map((_) => _.ta?.id)
        .compact(),
    }
    ...
    return DatatableUtils.buildColumns<ShelterEntity>([
      {
        ...
        id: 'ntaForm',
        options: () => [
          {value: 'exist', label: m._shelter.taRefOk},
          {value: 'notexist', label: m._shelter.taRefNok},
        ],
        render: (_) => {
          return {
            tooltip: null,
                ...
                {map(_.nta, (answer) => (
                  <>
                    <TableIconBtn
                      tooltip={m.view}
                      children="visibility"
                      onClick={() => ctxAnswers.openView({answer, formId: KoboIndex.byName('shelter_nta').id})}
                    />
                    <TableIconBtn
                      tooltip={m.edit}
                      href={ctx.asyncEdit(KoboIndex.byName('shelter_nta').id, answer.id)}
                      target="_blank"
                      children="edit"
                    />
                  </>
                  ...
                  <>
                    <Box sx={{display: 'flex', alignItems: 'center'}}>
                      <TableIcon color="error" sx={{mr: 0.5}}>
                        error
                      </TableIcon>
                      <Box>
                        <Txt block size="small" sx={{marginBottom: '-5px'}}>
                          TA ID: <Txt bold>{_.ta?.id}</Txt>
                        </Txt>
                        <Txt block size="small" sx={{marginBottom: '-2px'}}>
                          NTA Ref: <Txt bold>{_.ta?.nta_id}</Txt>
                        </Txt>
                      </Box>
                    </Box>
                  </>
      ...
      },
      {
        ...
        renderQuick: (_) =>
          _.nta ? (_.nta.street ?? '') + ' ' + (_.nta.house_number ?? '') + ' ' + (_.nta.building_number ?? '') : '',
      },
      {
        id: 'apartment_number',
        type: 'number',
      ...
      },
      {
        ...
        renderQuick: (_) => {
          if (!_.nta) return
          const items = seq([_.nta.ben_det_surname_l, _.nta.ben_det_first_name_l, _.nta.ben_det_pat_name_l]).compact()
      ...
      },
      {
        ...
        options: () => [
          {value: 'exist', label: m.exist},
          {value: 'not_exist', label: m.notExist},
        ],
        render: (_) => {
          return {
                ...
                {_.nta?.attachments && (
                  <>
                    <KoboAttachedImg
                      answerId={_.nta.id}
                      formId={KoboIndex.byName('shelter_nta').id}
                      attachments={_.nta.attachments}
                      fileName={_.nta.doc_available_yes1}
                    />
                    <KoboAttachedImg
                      answerId={_.nta.id}
                      formId={KoboIndex.byName('shelter_nta').id}
                      attachments={_.nta.attachments}
                      fileName={_.nta.doc_available_yes2}
                    />
                    <KoboAttachedImg
                      answerId={_.nta.id}
                      formId={KoboIndex.byName('shelter_nta').id}
                      attachments={_.nta.attachments}
                      fileName={_.nta.doc_available_yes3}
                    />
                    <KoboAttachedImg
                      answerId={_.nta.id}
                      formId={KoboIndex.byName('shelter_nta').id}
                      attachments={_.nta.attachments}
                      fileName={_.nta.doc_available_yes4}
                    />
                    <KoboAttachedImg
                      answerId={_.nta.id}
                      formId={KoboIndex.byName('shelter_nta').id}
                      attachments={_.nta.attachments}
                      fileName={_.nta.doc_available_yes5}
                    />
                  </>
                )}
              </Box>
      ...
      },
      {
        ...
        width: 0,
        typeIcon: null,
        subHeader: selectedNta.length > 0 && (
          <TableEditCellBtn
            onClick={() =>
          ...
          return {
            ...
            label: map(row.nta, (nta) => {
              return (
                ...
                >
                  {(value, onChange) => (
                    <SelectDrcProjects label={null} value={value} onChange={onChange} options={shelterDrcProject} />
                  )}
                </DebouncedInput>
              )
            }),
      ...
      },
      {
        ...
        type: 'select_one',
        options: () => [
          {value: 'true', label: m._shelter.taFilled},
          {value: 'false', label: m._shelter.taNotFilled},
        ],
        render: (_) => {
          return {
            tooltip: null,
            ...
            label: map(_.ta, (form) => (
              <>
                <TableIconBtn
                  tooltip={m.view}
                  children="visibility"
                  onClick={() => ctxAnswers.openView({formId: KoboIndex.byName('shelter_ta').id, answer: form})}
                />
                <TableIconBtn
                  tooltip={m.edit}
                  href={ctx.asyncEdit(KoboIndex.byName('shelter_ta').id, form.id)}
                  target="_blank"
                  children="edit"
                />
              </>
              ...
              <TableIconBtn
                ...
                target="_blank"
                href={
                  ctx.ta.schema.schema.deployment__links.url +
                  '?' +
                  objectToQueryString({
                    'd[nta_id]': _.nta!.id,
                    'd[interwiever_name]': ctx.nta.schema.translate
                      .choice('enum_name', _.nta?.enum_name)
                      ?.replaceAll(' ', '_'),
                    ...
                    'd[ben_det_addres]': _.nta
                      ? ((_.nta.street ?? '') + ' ' + (_.nta.house_number ?? '') + ' ' + (_.nta.building_number ?? ''))
                          .trim()
                          .replaceAll(' ', '_')
                      : '',
      ...
      },
      {
        ...
        typeIcon: null,
        options: () => [
          DatatableUtils.buildCustomOption(
            ShelterTaPriceLevel.Light,
            <>
              <TableIcon color="success">looks_one</TableIcon> {ShelterTaPriceLevel.Light}
            </>,
          ),
          DatatableUtils.buildCustomOption(
            ShelterTaPriceLevel.Medium,
            <>
              <TableIcon color="warning">looks_two</TableIcon> {ShelterTaPriceLevel.Medium}
            </>,
          ),
          DatatableUtils.buildCustomOption(
            ShelterTaPriceLevel.Heavy,
            <>
              <TableIcon color="error">looks_3</TableIcon> {ShelterTaPriceLevel.Heavy}
            </>,
          ),
        ],
        render: (row: ShelterEntity) => {
  ...
  const allowedData = useMemo(() => {
    if (ctx.allowedOffices.length === 0) return ctx.data.mappedData
