...
export const PanershipPanelDonor = ({data}: {data: Seq<PartnershipData>}) => {
  const {m} = useI18n()
  const [mode, setMode] = useState<'project' | 'donor'>('donor')
  return (
    <SlidePanel>
      <ScRadioGroup inline dense value={mode} onChange={setMode}>
        <ScRadioGroupItem hideRadio value="donor">
          {m.donor}
        </ScRadioGroupItem>
        <ScRadioGroupItem hideRadio value="project">
          {m.project}
        </ScRadioGroupItem>
      </ScRadioGroup>
      {fnSwitch(mode, {
        donor: (
          <ChartBarSingleBy
            ...
            label={Partnership_partnersDatabase.options.Donor}
            by={(_) => _.Donor!}
          />
        ),
        project: (
