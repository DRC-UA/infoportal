    ...
    <Box sx={{display: 'flex', mb: 1, alignItems: 'flex-start', ...sx}} {...props}>
      ...
      <Box>
        <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
          <Txt size={'small'} truncate bold>
            {partner.computed.name ?? partner.id}
          </Txt>
        </Box>
        <Txt color="hint" truncate block size="small">
          {schema.translate.choice('Type_of_organization', partner.Type_of_organization)}
        </Txt>
