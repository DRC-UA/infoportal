...
export const usePartnershipData = () => {
  ...
  const mappedData = useMemo(() => {
    ...
    fetcherPartnersDb.get.data.forEach((d) => {
      const oblast: OblastName[] | undefined = d.Which_oblasts_does_t_t_and_has_experience?.map((_) =>
        fnSwitch(_, {
      ...
      res.push({
        ...
        sectors: d.Which_sectors_does_the_organiz?.map((_) =>
          fnSwitch(_, {
            nfi: DrcSector.NFI,
            wash: DrcSector.WaSH,
            protection: DrcSector.GeneralProtection,
            ...
            gbv: DrcSector.GeneralProtection,
            eore: DrcSector.EORE,
          }),
        ),
