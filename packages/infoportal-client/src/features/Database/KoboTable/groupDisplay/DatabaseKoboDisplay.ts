...
import {colorRepeatedQuestionHeader, columnBySchemaGenerator, ColumnBySchemaGeneratorProps} from '../columns/columnBySchema'
    ...
    switch (display.repeatAs) {
      ...
      case 'rows': {
        ...
        const repeatGroupColumns = columnBySchemaGenerator({
          ...
          m,
          t,
        }).getByQuestions(group.questions).map(_ => {
          _.styleHead = {
            background: colorRepeatedQuestionHeader(t)
          }
          return _
