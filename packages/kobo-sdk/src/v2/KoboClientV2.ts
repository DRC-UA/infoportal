...
export class KoboClientV2 {
  ...
  readonly edit = (formId: Kobo.FormId, answerId: Kobo.SubmissionId) => {
    return this.api.get<{url: string; detail?: string}>(
      `/v2/assets/${formId}/data/${answerId}/enketo/edit/?return_url=false`,
    )
  }
  ...
  readonly getAnswers = async (
    form: Kobo.Form.Id,
    params: Kobo.Submission.Filter = {},
  ): Promise<Kobo.Paginate<Kobo.Submission>> => {
    return await this.getAnswersRaw(form, params).then((res) => {
      return {
        ...res,
        results: res.results
          .map((_) => {
